...
class MyDroneFirst(DroneAbstract):
    ...
    def __init__(self,
        ...
        self.state = self.Activity.WALL_FOLLOWING
        self.states = {self.Activity.SEARCHING_WOUNDED: self.searching_wounded,
                    ...
                    self.Activity.WALL_FOLLOWING: self.wall_following
                    }
...
################
    ...
    def update_state(self,found_wounded,found_rescue_center):    
        ...
        """
        if self.state != self.Activity.FOLLOW_PATH and self.state != self.Activity.WALL_FOLLOWING:
            self.index += 1
            self.index = self.index % len(self.state_machine)
            self.state = self.state_machine[self.index]
        elif self.state == self.Activity.WALL_FOLLOWING:
            # Condition pour quitter le suivi de mur
            if self.pathTracker.isFinish(self.path):  # Exemple de condition
                self.index += 1
                self.index = self.index % len(self.state_machine)
                self.state = self.state_machine[self.index]
        return None
    ...
    def control(self): # BOUCLE PRINCIPALE
        ...
        # Condition pour passer à l'état WALL_FOLLOWING
        right_dist = min([dist for dist, angle in zip(lidar_values, lidar_rays_angles) if np.pi / 4 < angle < 3 * np.pi / 4])
        if right_dist < 100:  # Si un mur est détecté à droite
            self.state = self.Activity.WALL_FOLLOWING

        # Appeler la fonction de contrôle en fonction de l'état actuel
        if self.state == self.Activity.WALL_FOLLOWING:
