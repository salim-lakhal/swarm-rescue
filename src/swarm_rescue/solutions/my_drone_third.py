...
from drone_modules.semantic_manager_solo import SemanticManager
...
class MyDroneFirst(DroneAbstract):
    ...
    def __init__(self,
        ...
        self.next_wounded_to_visit = None
        self.frontiers = []
...
##################
    ...
    def execute(self):
        ...
        if self.is_path_finish :
            if not self.found_wounded and np.linalg.norm(self.next_wounded_to_visit-self.pose.position) < 20:
                self.semanticManager.removeWoundedFromList(pos=self.next_wounded_to_visit)
            self.state = self.Activity.SEARCHING_WOUNDED
            return command
    ...
    def searching_wounded(self):
        ...
        if (self.is_path_finish and not self.found_wounded and self.iter > 5 * (self.identifier+1)):
            ...
            if n_cluster == 0 or self.grid.get_visited_ratio() > 0.95 or (self.frontiers is None):
                if self.list_wounded:
                    self.next_wounded_to_visit = self.list_wounded[0]
                    self.path_planning([(self.next_wounded_to_visit[0] + self.size_area[0]/2)/20,(self.next_wounded_to_visit[1] +self.size_area[1]/2)/20])
                    self.state = self.Activity.EXECUTE
                    return None
                return self.wall_following()
