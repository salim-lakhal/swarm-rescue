...
from drone_modules.filter import *
...
class MyDroneFirst(DroneAbstract):
    ...
    def __init__(self,
        ...
        self.data_true = np.empty((0,2))
        self.data_measured = np.empty((0,2))
    ...
    def control(self): # BOUCLE PRINCIPALE
        ...
        if self.iter == 0:
            self.filter = KalmanFilter(self.measured_gps_position())
        
        self.pose.position = self.true_position()#self.filter.fk(self.measured_gps_position(),self.measured_velocity())
        self.pose.orientation = self.true_angle()
        lidar_values = self.lidar_values()
        lidar_rays_angles = self.lidar_rays_angles()
        
        v = self.measured_velocity()
        ...
        self.data = np.vstack((self.data, self.pose.position))
        self.data_true = np.vstack((self.data_true, self.true_position()))
        self.data_mesured = np.vstack((self.data_measured, self.true_position()))

        
        with open("data_measure_position.csv",'a') as f:
            np.savetxt(f, self.measured_gps_position().reshape(1, 2), delimiter=',', fmt='%f')

        with open("data_true_position.csv",'a') as f:
            np.savetxt(f, self.true_position().reshape(1, 2), delimiter=',', fmt='%f')
        ...
        #obstacles = np.array([[10,2,-6,-14,-23],[0,0,0,0,0]])
        ...
        if self.state is self.Activity.INITIAL:
            ...
            #path = self.planning.potential_field_planning(295,118,-100,0,obstacles[0],obstacles[1],8,50)
            ...
            #path = self.planning.potential_field_planning(self.pose.position[0],self.pose.position[1],-100,0,obstacles[0],obstacles[1],8,15)
            #self.path._poses = path.T
